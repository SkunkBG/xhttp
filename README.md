...
## Тест настоящим клиентом

Если сайт работает и Caddy проксирует в Xray, но клиенты не подключаются — проверьте связку изнутри:

```bash
bash <(curl -Ls https://raw.githubusercontent.com/SkunkBG/xhttp/main/xhttp-clienttest.sh)
```

Скрипт поднимает локальный Xray-клиент с заведомо правильными параметрами и пробует выйти в интернет через ваш же сервер. Понадобится UUID любого активного пользователя из панели.

- **Туннель поднялся** → сервер исправен, дело в настройках хоста в панели.
- **Не поднялся** → покажет лог клиента с конкретной причиной.

### Важно про Security в хосте

Инбаунд Xray работает с `"security": "none"` — TLS терминирует Caddy. Панель может сгенерировать клиентскую ссылку без TLS. В настройках хоста **Security нужно выставить в TLS принудительно**, а Port — в **443**, а не в порт инбаунда (8001).
